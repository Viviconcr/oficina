            ...
            for msg in data['messages']:
                if 'quotedMsgId' in msg and msg['quotedMsgId']:
                    if 'chatId' in msg and msg['chatId']:
                        ...
                        mobile = '+' + str(country_code) + ' ' + str(int(str(mobile_number)[:5])) + ' ' + str(
                            int(str(mobile_number)[-5:]))
                        res_partner_obj = res_partner_obj.sudo().search([('mobile', '=', mobile)], limit=1)
                        mail_message_id = mail_message_obj.sudo().search([('whatsapp_message_id', '=',msg['quotedMsgId'])],
                                                                         limit=1)
                        if mail_message_id.model == 'mail.channel' and mail_message_id.res_id:
                            channel_id = mail_channel_obj.sudo().search(
                                [('id', '=', mail_message_id.res_id)])
